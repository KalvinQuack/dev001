...
#define _PINOUT_C_
    ...
    #define PORT 0xF0
    #define PIN  0x0F
    #define GPIO_PORT_PIN   (port, pin)     (((port)<< 8)|(pin))
    #define GET_GPIO_PORT   (val)           ((val)>>8)
    #define GET_GPIO_PIN    (val)           ((val)&0xFF)
    typedef struct {
     ...
     */
    const uint32_t GPIO_inputMap[DIGITAL_INPUT_TOTAL][2] = 
    {
    ...
    const uint32_t GPIO_outputMap[DIGITAL_OUTPUT_TOTAL][2] = 
    {
